...
function createNewDashboard() {
    ...
    const newDash = {
        ...
        emoji: "📄",
        folderId: null, // Starts in "Lone Dashboards"
        isLocked: true,
        blocks: [{
            id: generateUniqueId(),
            type: 'header',
...
function renderDashboardList() {
    ...
    folders.forEach(folder => {
        ...
        folderEl.innerHTML = `
            ...
            <div class="folder-content" data-folder-id="${folder.id}">
                ${folderDashboards.map(d => createDashboardCardHTML(d)).join('')}
            </div>
        `;
        container.appendChild(folderEl);
    });

    // Render Lone Dashboards
    const loneDashboards = dashboards.filter(d => !d.folderId || d.folderId === null);
    ...
    loneEl.innerHTML = `
        ...
        <div class="folder-content" data-folder-id="null">
            ${loneDashboards.map(d => createDashboardCardHTML(d)).join('')}
        </div>
    `;
    container.appendChild(loneEl);


    // Make cards clickable to open the sheet
    ...
    // Initialize drag-and-drop for dashboards
...
function renderDashboardSheet(dashboard) {
    ...
    // Attach listeners for editable content and delete buttons if unlocked
    if (!isLocked) {
        ...
        container.querySelectorAll('.block-content[contenteditable="true"]').forEach(el => {
            el.addEventListener('blur', (e) => {
                ...
                if (block) {
                    block.content = e.target.innerHTML; // Use innerHTML to save formatting like lists
        ...
        container.querySelectorAll('.delete-block-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                if (confirm('Are you sure you want to delete this block?')) {
                    ...
                    renderDashboardSheet(dashboard); // Re-render the sheet
                    saveAppState();
                }
            });
        });
...
function closeDashboardSheet() {
    ...
    renderDashboardList(); // Re-render to reflect any changes
}
...
function loadInitialDashboardData() {
    folders = [{
        ...
        id: 'folder2',
        name: '📂 Party Characters'
    }];
    dashboards = [{
        id: 'd1',
        title: 'Turnden Forewit',
        description: 'Halfling rogue, good with locks.',
        emoji: '🧙‍♂️',
        folderId: 'folder1',
        isLocked: true,
        blocks: [{
            id: generateUniqueId(),
            type: 'header',
            title: '🔲 Block 1: Header',
            content: 'Halfling Rogue, Master of Locks'
        }, {
            id: generateUniqueId(),
            type: 'stats',
            title: '🔲 Block 2: Stats Box',
            content: '🎲 AC: 15 🎯 HP: 24/28 ⚔️ Attack: +5 🧪 DEX +4 💭 INT +2'
        }, {
            id: generateUniqueId(),
            type: 'notes',
            title: '🔲 Block 3: Notes',
            content: '“Found a strange orb in the ruins.”<ul><li>Investigate with wizard</li><li>Check for traps next time</li></ul>'
        }, ]
    }, {
        id: 'd2',
        title: 'Lord Vareth',
        description: 'Ancient black dragon. Tricky.',
        emoji: '🐉',
        folderId: 'folder1',
        isLocked: true,
        blocks: []
        ...
        id: 'd3',
        title: 'Seralyn the White',
        description: 'Elf cleric. Healing focus.',
        emoji: '🧝',
        folderId: 'folder2',
        isLocked: true,
        blocks: []
        ...
        id: 'd4',
        title: 'The Crimson Fang',
        description: 'Vampire noble in disguise.',
        emoji: '🧛',
        folderId: null,
        isLocked: true,
        blocks: []
