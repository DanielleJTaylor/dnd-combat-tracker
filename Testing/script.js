...
function applyHpChange() {
  if (!activeHpCombatantId) return;

  const target = findCombatantById(activeHpCombatantId);
  if (!target) return;

  const healAmount = parseInt(document.getElementById('healInput').value, 10) || 0;
  const damageAmount = parseInt(document.getElementById('damageInput').value, 10) || 0;

  const oldHp = target.hp;
  target.hp = Math.min(target.maxHp, Math.max(0, oldHp + healAmount - damageAmount));

  logChange(`${target.name} HP changed: ${oldHp}/${target.maxHp} → ${target.hp}/${target.maxHp}`);
